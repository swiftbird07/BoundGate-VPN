...
// the tray app of a standard user) that group may connect as well, like each
// of users (accounts). Windows checks write access to the socket file on
// connect.
func protect(socketPath, group string, users []string) error {
	sddl := "D:P(A;;GA;;;SY)(A;;GA;;;BA)"
	for _, name := range append([]string{group}, users...) {
		if name == "" {
			continue
		}
		...
		if err != nil {
			...
			if err != nil {
				return fmt.Errorf("ipc: socket access for %q: %w", name, err)
			}
		}
