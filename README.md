...
Status: prototype, milestone M8 stage 1 (macOS menu-bar app, see `docs/MACOS-APP.md`; fully containerized deployment from one prebuilt image (`gitlab.net407.com/sbh/boundgate:latest`, amd64+arm64, CI-built): all-in-one kit with control plane and hub sharing port 443 through `boundgate-mux`, single-node kit for every further machine, built-in Let's Encrypt, see `docs/DEPLOY.md`; TPM 2.0 device keys incl. VM vTPMs, `hardware_bound` as a signed binding field, see `docs/TPM.md`; macOS endpoint, see `docs/MACOS.md`; node model: control plane on port 443,
